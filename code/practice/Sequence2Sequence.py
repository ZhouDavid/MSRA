...
def translator_test_error(z, trainer, input_vocab_dim, label_vocab_dim, debug_output=False):
    ...
    test_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "en_fr", "test.ctf")
...
def sequence_to_sequence_translator(debug_output=False, run_test=False):
    fven=open('D:\MSRA\code\practice\old_en_freq.txt')
    fvfr=open('D:\MSRA\code\practice\old_fr_freq.txt')
    ven=[line.split(' ')[0] for line in fven.readlines()]
    vfr=[line.split(' ')[0] for line in fvfr.readlines()]
    v_total=list(set(ven) | set(vfr))
    i2w={ i:ch for i,ch in enumerate(v_total)}
    input_vocab_dim = len(v_total)
    label_vocab_dim = len(v_total)
    ...
    # setup data
    train_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "en_fr", "train.ctf")
    valid_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "en_fr", "valid.ctf")
    ...
    vocab_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "en_fr", "cmudict-0.7b.mapping")
    ...
    z.save_model("/media/root/cdd278c0-f2d0-4bda-86b4-38a783149545/yanxu/seq2seq.dnn")
    z.restore_model("/media/root/cdd278c0-f2d0-4bda-86b4-38a783149545/yanxu/seq2seq.dnn")
